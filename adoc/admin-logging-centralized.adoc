...
[WARNING]
====
Currently, Rsyslog can cause segfault as described in https://github.com/rsyslog/rsyslog/issues/4200 as result of both `imjournal` and `imfile` input modules enabled. To avoid Rsyslog crash, `imfile` and `imjournal` need to be mutually exclusive, which mean we need to configure helm charts to one of these 
...
----
 ...
 If required to send both `imjournal` and `imfile`, Rsyslog need to install on hosts to send `imjournal` log and helm chart deploy on Kubernetes to send `imfile` log.
====
