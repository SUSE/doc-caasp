...
# end::tf_vmware[]
<17> `wait_for_guest_net_routable`: true or false to disable the routable check. Default is true.
*Note*: This should only be changed if terraform times out while creating VMs as mentioned on the upstream bug: https://github.com/hashicorp/terraform-provider-vsphere/issues/1127
...
----
<1> `stack_name`: Prefix for all machines of the cluster spawned by terraform.
<2> `masters`: Number of master nodes to be deployed.
<3> `workers`: Number of worker nodes to be deployed.
<4> `authorized_keys`: List of ssh-public-keys that will be able to log into
the deployed machines.
<5> `caasp_registry_code`: {productname} Product Key for registering
the product against {scc}.
<6> `caasp_registry_code`: register against a local SUSE Repository
Mirroring Server.
<7> `peer_vpc_ids`: List of already existing VPCs to join via dedicated VPC
peering links.
<8> `iam_profile_master`: Name of the IAM profile to associate with the control
