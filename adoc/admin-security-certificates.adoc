...
+
Please refer to xref:trusted_server_certificate[] on how to sign the trusted certificate. The `server.conf` for IP.1 is the original SAN IP address if present, DNS.1 is the original SAN DNS if present.
+
...
{productname} renews all certificates excluding `oidc-dex` and `oidc-gangway` automatically during the control plane update, see xref:handling_updates[].
...
+
Please refer to xref:self_signed_server_certificate[] on how to sign the self signed server certificate. The default kubernetes CA certificate and key are located at `/etc/kubernetes/pki/ca.crt` and `/etc/kubernetes/pki/ca.key`. The `server.conf` for IP.1 is the original SAN IP address if present, DNS.1 is the original SAN DNS if present.
+
.. Trusted CA certificate
+
Please refer to xref:trusted_server_certificate[] on how to sign the trusted server certificate. The `server.conf` for IP.1 is the original SAN IP address if present, DNS.1 is the original SAN DNS if present.
...
If you are using proper trusted 3rd-party CA signed certificates, skip the following steps and refer to xref:trusted_server_certificate[].
