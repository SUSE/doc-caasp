...
Administrators can provide custom CA certificates during cluster deployment and
decide which CA components to replace (multiple CA certificates) or
if to replace all with a single CA certificate.
...
Enduser faces oidc-dex and oidc-gangway server to do authentication and authorization, administrators might want to replace these server certificates issue by a trusted CA certificate after cluster deployment. Therefore, the enduser does not need to add self-signed CA certificates manually to the trust certificate chain.

[WARNING]
====
Due to the trusted CA certificate key is not handle by skuba, the administrator have to handle to server certificate rotation manually if the certificate expires.
====
...
. Sign the oidc-dex server certificate with the trusted CA certificate, make sure that the signed server certificate SAN is the same as the origin. To get the original SAN IP address(es) and DNS(s), run:
+
[source,bash]
----
...
. Sign the oidc-gangway server certificate with the trusted CA certificate, make sure that the signed server certificate SAN is the same as the origin. To get the original SAN IP address(es) and DNS(s), run:
+
[source,bash]
----
...
{productname} renews all certificates excludes oidc-dex ang oidc-gangway automatically during the control plane update, see <<handling_updates>>.
