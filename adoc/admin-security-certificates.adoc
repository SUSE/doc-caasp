  ...
  template:
    ...
    spec:
      ...
      - image: registry.suse.com/caasp/v4/cert-exporter:2.2.0
        ...
        - --secrets-exclude-glob=tls.key // <4>
...
<2> Namespace of the custom secret, remove this if you want to scrapes multiple namespaces.
<3> Specifies the secret data keys to scrape. You could create multiple secret data keys to scrape by creating multiple flag `secrets-include-glob`.
<4> Specifies the secret data keys to not scrape. You could create multiple secret data keys to not scrape by creating multiple flag `secrets-exclude-glob`.
<5> Specifies the secret label selector. You could create multiple label selectors by creating multiple flag `--secrets-label-selector`.
<6> Specifies the secret annotation selector. You could create multiple annotation selectors by creating multiple flag `--secrets-annotation-selector`.
