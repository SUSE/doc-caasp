...
skuba uploads the local OIDC CA certificate to remote path specified in the local file `kubeadm-init.conf` key `oidc-ca-file`. At the time installing the add-on, skuba generates the OIDC server certificates and keys which are signed by the provided OIDC CA certificate and key pair and stored to its Secret resource.

** Without a Trusted CA Key:
+
Use command `skuba cert generate-csr` to generates the OIDC server CSRs and keys in `<my-cluster>/pki` folder.
After the CA signed the CSRs and issued the server certificates, puts the OIDC CA certificate and the OIDC server certificates in `<my-cluster>/pki` folder.
+
[source,bash]
----
mkdir -p <CLUSTER_NAME>/pki
...
+
After this process, bootstrap the cluster with `skuba node bootstrap`.
skuba uploads the local OIDC CA certificate to remote path specified in the local file `kubeadm-init.conf` key `oidc-ca-file`. At the time installing the add-on, skuba uploads the OIDC CA certificate and OIDC server certificate and key pair to its Secret resource.
