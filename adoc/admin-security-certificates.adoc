...
. Sign the `metrics-server-cert` server certificate with the default kubernetes CA certificate
+
...
. Create a secret manifest file [path]`oidc-metrics-server-cert.yaml` and update the secret data `ca.crt`, `tls.crt`, and `tls.key` with base64; encoded with CA certificate, signed `metrics-server` server certificate and key respectively.
+
----
apiVersion: v1
kind: Secret
