...
[WARNING]
====
Once the cert-manager have issued a certificate to the secret, if you change the certificate inside the secret manually, or you manually change the current certificate `duration` to a value inferior at the value `renewBefore`, the certificate won't be renewed immediately but schedule to renew near the certificate expiry date.

This is because the cert-manager is not designed to pick up changes you make to the certificate in the secret. 
However, this behavior will change in the future where cert-manager check the certificate inside the secret.
====
