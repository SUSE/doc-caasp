...
== Use Custom Trusted CA Certificate

=== Use Custom Trusted CA Certificate During Cluster Deployment
...
* With The Trusted CA Key
+
Please refer to <<addon_certificates_rotation>> on how to use cert-manager and reloader to issue `oidc-dex` and `oidc-gangway` certificate signed by trusted CA certificate/key.

* Without The Trusted CA Key
+
[WARNING]
====
Because the custom trusted CA certificate is not in {kube} cluster, administrators must handle server certificate rotation manually before the certificate expires.
...
{productname} renews all certificates excluding `oidc-dex` and `oidc-gangway` automatically during the control plane update, see <<handling-updates>>.
...
[[addon_certificates_rotation]]
=== Addon certificates rotation

The addon certificates can be automatically rotated by leveraging the open-source solutions `cert-manager` and `reloader`. `cert-manager` is for automatically rotating the certificates stored in Secrets, and `reloader` is for watching and reconciling the updated Secrets to execute rolling upgrade the affecting addon Deployments or DaemonSet.

* Prerequisites

. To let reloader automatically rolling upgrade the addon Deployments or DaemonSet, we need to label the addons:
+
[source,bash]
----
...
. Upload the Kubernetes CA cert/key pair to Secret in namespace kube-system:
+
[source,bash]
----
...
[NOTE]
====
If you want to use custom trusted CA certificate/key to sign the certificate, upload to the secret resource.
...
. Install reloader by helm chart:
+
[source,bash]
----
...
+
The cert-manager CA issuer is a Kubernetes resource that represents a certificate authority (CA) that is able to generate signed certificates by honoring certificate signing requests (CSR). Each cert-manager certificate resource requires one referenced issuer in the ready state to attempt to honor CSR requests.
+
[NOTE]
====
An `Issuer` is a namespaced resource, and it can not issue certificates to the `Certificate` resources in different namespaces.
...
====
+
Create a CA issuer called `kubernetes-ca` that will sign certificates based on a private key. The certificate stored in the secret `kubernetes-ca` can then be used to trust newly signed certificates by the kubernetes-ca issuer.
+
[source,bash]
----
cat << EOF > issuer.yaml
...
[NOTE]
====
If you want to use custom trusted CA certificate/key to sign the certificate, create a custom trust CA issuer.
...
----
<1> Certificate default duration, format (XhYmZs).
<2> Certificate renew before expiration duration, format (XhYmZs).
...
+
The dnsNames and ipAddressed fields specify a list of Subject Alternative Names to be associated with the certificate.
+
...
[NOTE]
====
If you want to use custom trusted CA Issuer/ClusterIssuer, change <5> to custom-trusted-ca and <6> to Issuer/ClusterIssuer.
====
