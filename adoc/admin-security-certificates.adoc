...
We use cert-export to monitors the node's on-disk certificates and addon Secret certificates. The cert-exporter exposes these certificates expirations to the cert-exporter `/metrics` endpoint. Then, the Prometheus server fetches the `/metrics` endpoint periodically.
