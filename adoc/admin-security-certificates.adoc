...
Administrators can provide custom CA certificates during cluster deployment and decide which CA components to replace
(multiple CA certificates) or if to replace all certificates with a single CA certificate.
...
. Sign the `oidc-dex` server certificate with the trusted CA certificate, make sure that the signed server certificate SAN is the same as the origin.
...
. Sign the `oidc-gangway` server certificate with the trusted CA certificate, make sure that the signed server certificate SAN is the same as the origin.
To get the original SAN IP address(es) and DNS(s), run:
+
[source,bash]
----
...
----
+
. Sign the `oidc-dex` server certificate with the CA certificate/key `/etc/kubernetes/pki/ca.crt` and `/etc/kubernetes/pki/ca.key`, make sure that the signed server certificate SAN is the same as the origin.
To get the original SAN IP address(es) and DNS(s), run:
+
[source,bash]
----
...
----
+
. Sign the `oidc-gangway` server certificate with the CA certificate/key `/etc/kubernetes/pki/ca.crt` and `/etc/kubernetes/pki/ca.key`, make sure that the signed server certificate SAN is the same as the origin.
To get the original SAN IP address(es) and DNS(s), run:
+
[source,bash]
----
