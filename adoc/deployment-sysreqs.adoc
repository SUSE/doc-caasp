...
The sizing of the (v)CPU can be calculated by adding up the base requirements, the estimated essential cluster addons (logging agent, monitoring agent, configuration management, etc) and the estimated CPU workloads:

* 0.250 (base requirements) + 0.250 (estimated cluster addons requirements) + estimated CPU workloads requirements

The sizing of the RAM can be calculated with the same principles:

* 1.2 GB (base requirements) + 500 MB (estimated cluster addons requirements) + estimated RAM workloads requirements
