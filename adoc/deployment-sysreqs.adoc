...
The {productname} needs an external authentication server like LDAP, AD, etc to validate the user's entity and grant different user roles or cluster role
permission by leveraging {kube} role-based access control (RBAC).
