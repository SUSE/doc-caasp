...
Some addon services is desired to have enough cluster nodes available for replicas to achieve high availabilty.

When the cluster is deployed with enough nodes for replica sizes, those service distributions will be balanced across cluster.

You can check the deployment replica size after node bootstrap. The number of cluster node should be equal or greater than the `DESIRED` replica size.
----
kubectl get rs -n kube-system
----

After deployment, if number of healthy nodes fall under replica size, service replicas once on the unhealthy node will show in `Pending` state until either the unhealthy node recovers or new node joined to cluster.

For cluster deployed with node number less than the default replica sizes. Services will still try to find a suitable node to run on. However it is likely you will see services all running on same node. Following are two methods for replica management if you wish to work with cluster below the default replica size.

Reduce replicas number:

. Reduce deployment replica size before node join. Adopt same method to increase the replica size before new node join if later have more available resource for new node. 
+
----
...
Check the `NAME` and `NODE` colume for duplicated pods.
+
----
