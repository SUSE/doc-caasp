...
The {productname} leverages {kube} role-based access control (RBAC) for authentication and will need to have an external authentication server such as LDAP, AD, etc to validate the user's entity and grant different user roles or cluster role permission.
