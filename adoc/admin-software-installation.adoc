...
Helm 2 is the default for {productname} 5.0.0. Helm 3 is offered as an alternate tool and may be installed in parallel to aid migration. Helm 3 may be selected by installing from zypper then selecting the alternative.
...
[WARNING]
Helm 2 is planned to end support in November 2020. Helm 3 is offered as an alternative in {productname} 5.0 and will become the default tool in the following release. For any upgraded installations, please follow the Migration Guide as soon as feasible.
...
* A system where `skuba` and `helm` version 2 have run from previously.
...
The following procedure is for migrating a CaaSP 5.0 deployment that has used Helm 2 to deploy applications.

. Install `helm3` package on the same location you normally run skuba commands (alongside the helm2 package)
+
----
...
----
.. After move, if you have installed any custom plugins then check they work fine with Helm 3. If needed, remove and re-add them as described in https://github.com/helm/helm-2to3
...
helm3 2to3 cleanup
----
. You may now uninstall the helm2 package and use `helm` command line from the helm3 package from now on.
+
----
