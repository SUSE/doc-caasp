...
When you want to make sure all the deployments you have are compatible with the new version, in an air gapped environment you have to follow the next steps.
...
. In a secure and trusted machine download the `swagger` file, that has included all APIs definitions for kubernetes, from the URL:
  ...
  `https://raw.githubusercontent.com/kubernetes/kubernetes/vX.XX.XXX/api/openapi-spec/swagger.json`

  Replace `vX.XX.XXX` with the desired version of kubernetes, e.g.: `v1.18.5`  
...
. Run the following {productname} command to test against that file:
  ...
  `skuba cluster upgrade check --kubernetes-version=X.XX.XX --swagger-dir=/path/to/containing/folder`
