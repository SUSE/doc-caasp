...
|KUBEADM|etcd|registry.suse.com/caasp/v5/etcd
|KUBEADM|api-server|registry.suse.com/caasp/v5/api-server
|KUBEADM|controller-manager|registry.suse.com/caasp/v5/controller-manager
|KUBEADM|scheduler|registry.suse.com/caasp/v5/scheduler
|KUBEADM|proxy|registry.suse.com/caasp/v5/proxy
...
=== Check for Deprecated APIs before an Upgrade to New Version in an air gapped deployment

When you want to make sure all the types kubernetes objects defined in your cluster are compatible with the new version, in an air gapped environment you have to follow the next steps.

. In a secure and trusted machine download the `swagger` file, that has included all APIs definitions for kubernetes, from the URL:

  `https://raw.githubusercontent.com/kubernetes/kubernetes/v<MAJOR.MINOR.PATCH>/api/openapi-spec/swagger.json`

  Replace `v<MAJOR.MINOR.PATCH>` with the desired version of {kube}, e.g.: `v1.18.5`  

. Copy the `swagger` file to a secure storage device.

. Connect the device in the air gapped environment.

. Copy the `swagger` file to a path in your system.

. Run the following {productname} command to test against that file:

  `skuba cluster upgrade check --kubernetes-version=v<MAJOR.MINOR.PATCH> --swagger-dir=/path/to/containing/folder`

  
This will show a list, if any, for deprecated APIs that are being used in your current cluster setup. Allowing you to be able to update accordingly also.
