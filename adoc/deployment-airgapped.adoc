...
+
xref:airgap-caasp_deployment[]
+
RPM Packages: xref:airgap-rpm_repository-client[]
+
Helm Charts: xref:airgap-helm_charts-client[]
+
Container Images: xref:airgap-container_registry-client[]
...
Refer to: xref:airgap-rpm_repository-update[].
...
[IMPORTANT]
====
To make use of the helm charts, you must complete xref:airgap-container_registry[].
====
...
Update the Helm chart repository by following the shared update procedure xref:airgap-update[].
...
+
For `--dest-creds` you must use the credentials you created during xref:airgap-container_registry-mirror[].
+
----
...
// .
// Refer to: xref:airgap-rpm_repository-client[].
