...
{rmt}
will provide a repository server that holds the packages and related metadata for {sls}; to install them like from the upstream repository.
...
----
sudo ``mkdir -p /etc/docker/registry/{auth,certs}`` sudo ``docker run --entrypoint htpasswd registry.suse.com/sles12/registry:2.6.2 -Bbn USERNAME PASSWORD \
...
Make sure to add the CA certificate of your {rmt}
...
From {yast}
register the node against the {rmt}
server.
...
Ensure the admin node is registered against {rmt}
, that will ensure the nodes that are provisioned by AutoYaST are registered against {rmt}
