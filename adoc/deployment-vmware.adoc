...
bare metal machines as described in xref:deployment_bare_metal[].
...
. Register the SLES15-SP1 system. Substitute `<CAASP_REGISTRATION_CODE>` for the code from xref:registration_code[].
+
----
...
----
. Register the {productname} module. Substitute `<CAASP_REGISTRATION_CODE>` for the code from xref:registration_code[].
+
----
...
[TIP]
====
To manually generate the unique `machine-id` please refer to: xref:machine-id-regen[].
====
