...
like sudo, vmware tools and adding the kubernetes dependencies.

Steps 1-3 may be skipped, if they were already performed during installation.
...
To obtain the required packages you can use the `CaaSP` product repositories to
...
Also adjust the file `registration.auto.tfvars` and add the `CaaS Platform`
