...
. Temporarily enable SSH access to one of the ESXi nodes in the vSphere WebUI as described at link:https://kb.vmware.com/s/article/2004746[]
. Log in to the node.
...
+
image::vmware_step5.png[]
. Now customize the hardware settings
+
image::vmware_step6.png[]
.. Select menu:CPU[2]
...
+
image::vmware_step7.png[]
. Confirm the partitioning changes.
+
image::vmware_step14.png[]
. Click "Next".
