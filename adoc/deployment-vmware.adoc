...
[IMPORTANT]
====
Jumbo frames must be enabled on the switch ports that the ESXi hosts are connected to, it will let ESXi hosts send larger packets over the physical network. Maximum transmission unit (MTU) can be set as value greater than 1500 bytes, where 9000 bytes is the maximum frame size that can be configured.
====
