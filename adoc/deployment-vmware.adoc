...
----
. Download the pre-built ISO image of SUSE SLES15-SP1 for VMWare from {jeos_product_page_url}
...
==== Setup by hand
For each VM deployment follow {ay} installation method used for deployment on Bare-metal machines as described at <<deployment.on.bare.metal>>.
