...
When use dex & gangway to do authenticated and authorizated, you might see an error message in the output of Web browser.
...
This message indicates that your {kube} cluster dex & gangway client secret is out of sync. Please update the dex & gangway ConfigMap use the same client secret.
...
Make sure the oidc's `secret` in `oidc-dex-config.yaml` equals to the `clientSecret` in `oidc-gangway-config.yaml`. Then, applying the updated ConfigMap.
