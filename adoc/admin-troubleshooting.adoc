...
[NOTE]
====
For more information about `etcd`, refer to xref:troubleshooting-etcd[].
====
...
If your cluster loses a node, for example due to failed hardware, remove the node as explained in xref:removing_nodes[].
Then add a new node as described in xref:adding_nodes[].
...
This means the tiller installation was secured via SSL/TLS as described in xref:helm_tiller_install[].
