...
====
    {
        ...
        "useManagedIdentityExtension": true, // <8>
...
<8> (optional) Use managed service identity for the virtual machine to access Azure ARM APIs. Set to false if not using managed identity authentication.
...
To use service principle authentication you need to <<cluster-bootstrap-azure-create-service-principle>> and provide `aadClientId`, `aadClientSecret` and configure `useManagedIdentityExtension` to `false`.
====
...
[[cluster-bootstrap-azure-create-service-principle]]
===== Create Service Principle
[IMPORTANT]
====
Ensure there is sufficient azure account permission. The account needs to be either a member of the `Owner` role, or have `Contributor` plus `User Access Administrator` roles.
====
...
====
    ...
    SUBSCRIPTION="<SUBSCRIPTION_NAME>" // <2>
    SUBSCRIPTION_ID=`az account list --output table | grep "${SUBSCRIPTION_NAME}" | rev | awk '{print $3}' | rev`

    az ad sp create-for-rbac --role="Contributor" --scopes="/subscriptions/${SUBSCRIPTION_ID}"
====
<1> The name of the resource group that the cluster is deployed in.
<2> The ID of the Azure Subscription that the cluster is deployed in.

. Record the `appId`, `password` and `tenant` to use in Azure cloud provider configuration.
+
====
    Creating a role assignment under the scope of "/subscriptions/00000000-0000-0000-0000-000000000000"
    {
      "appId": "00000000-0000-0000-0000-000000000000", // <1>
      "displayName": "azure-cli-2020-06-12-00-51-10",
      "name": "http://azure-cli-2020-06-12-00-51-10",
      "password": "P`_g:V\\v[cJ6fAu~/2Ak7$F1w1C@[lRi", // <2>
      "tenant": "000000000000000000-0000-000000000000" // <3>
    }
====
<1> The appId applies to aadClientId.
<2> The password applies to aadClientSecret.
<3> Then tenant applies to tenantId.
