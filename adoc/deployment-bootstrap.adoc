...
Please be aware that this might result in incompatibility with your previously running workloads,
unless you explicitly define the additional Linux capabilities required on top of CRI-O defaults.
...
If you wish to customize addons to the {kube} cluster you can do so by placing configuration files into the
`my-cluster/addons/crio.conf.d/` directory. The files will be deployed to `/etc/crio/crio.conf.d/` on each resulting cluster node.

You can also customize the {crio} registries configuration by creating `my-cluster/addons/containers/registries.conf` this will
replace the `etc/containers/registries.conf` on each node after bootstrapping.
