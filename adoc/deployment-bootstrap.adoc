...
Some nodes might run specially treated workloads (pods) on a node.

To prevent downtime of those workloads and its node, it is possible to flag a pod by a `--blocking-pod-selector=<NAME OF POD>`. Any node running this workload will not be rebooted via `kured` and needs to be rebooted manually.
