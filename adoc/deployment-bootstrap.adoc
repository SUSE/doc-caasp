...
[IMPORTANT]
====
All virtual machines must exist in a folder and provide as the `folder` variable in the `vsphere.conf` before bootstrap.
...
For clusters provisioned by any other method, ensure to create and move all cluster node virtual machines to a folder.
====
