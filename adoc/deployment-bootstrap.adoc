...
===== Network Settings
*It is vital that you adjust the networking settings* inside `kubeadm-init.conf`.
This must be performed before the bootstrap and planned carefully,
...
The subnet also mustn't conflict with services from outside of the cluster, for example external databases, file services, etc.
