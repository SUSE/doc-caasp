...
[TIP]
====
During cluster bootstrap, {skuba} generates root CA certficiate by default. Administrators could deploy {kube} cluster with your own custom root CA certificate. Please refers to the _{productname} Admin Guide_.
====
+
[IMPORTANT]
.Custom root CA certificate
====
Plan carefully for the custom root CA certificate, it is not possible to reconfigure the custom root CA certificate once the deployment is complete.
====
+
[source,bash]
----
