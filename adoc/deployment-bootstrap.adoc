...
By default, there is no default user when `skuba` connects to the nodes.
However, when a bastion host is used to connect to the nodes and if no user
is provided with the flag `--bastion-user`, then it will default to the user
provided with `--user`.
...
. Before deploying, it is important to know that all the SSH connections made
with the `skuba node (bootstrap, join, upgrade apply)` commands below can be done through a bastion.
+
