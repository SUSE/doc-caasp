...
The default deployment aims for an HA Kubernetes service. In order to
achieve HA it's required to run several control planes.
...
So we see that in order for HA to also be fault tolerant it's worth
having an odd number of control planes. 3 master nodes is the minimum
in order to tolerate a complete loss of one control plane node.

Control planes is only part of the whole picture. Most components will
talk to the API Server, and we need to expose the API Server in all
masters to the clients and the rest of the cluster components.

For this, a load balancer is required. The load balancer will point to
all the control plane nodes, and the clients and the rest of the
Kubernetes components will talk to the load balancer, that will
perform health checks against all the control planes, maintaining an
active pool of backends.

If only one load balancer is deployed, since it's a single component
running in a single machine, it's a single point of failure. That's
why more than one load balancer is required for a complete HA
solution. In this case as long as there are more than one load
balancer the solution is HA and fault tolerant.
...
* CNI: Container Network Interface. The default plugin providing CNI
...
* Network policies: Network policies allow for security in terms of
...
* Kube proxy: The kube proxy is a service that runs containerized in
...
Certificates are stored under /var/lib/kubelet/pki on the worker
nodes.
...
Cluster lifecycle is managed using `caaspctl`. It enables you to
...
```
~/clusters$ caaspctl cluster init --control-plane 10.86.3.149 my-cluster
...
```
~/clusters/my-cluster$ caaspctl node bootstrap --user sles --sudo --target <IP address/FQDN> <nodename>
```
...
the following `caaspctl` command:

```
~/clusters/my-cluster$ caaspctl node join --role master --user sles --sudo --target <IP address/FQDN> <nodename>
```
...
does not exist, `caaspctl` will read the `kubeadm-join.conf.d/master.conf.template`
instead and will create this file automatically when `caaspctl node
join` is called.
...
```
~/clusters/my-cluster$ caaspctl node join --role worker --user sles --sudo --target <IP address/FQDN> <nodename>
```
...
new node prior to joining it (a similar procedure to how
`kubeadm-init.conf` file behaves when bootstrapping). If this file
does not exist, `caaspctl` will read the `kubeadm-join.conf.d/worker.conf.template`
instead and will create this file automatically when `caaspctl node
join` is called.

==== Removing nodes from the cluster

Removing nodes from the cluster requires you to execute `caaspctl`
...
```
~/clusters/my-cluster$ caaspctl node remove <nodename>
```
