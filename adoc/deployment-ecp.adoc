...
`caaspctl` tool to bootstrap the {kube} cluster on top of those.

=== Preparation

Both `caaspctl` and `terraform` (along with its configuration examples) are provided by the `SUSE-CaaSP-Management` pattern.

. Login and register
+
Activate your {suse} SLES15-SP1 and {productname} {productmajor} subscription, activating the containers module as well:
+
----
