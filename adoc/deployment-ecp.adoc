...
----
+
. Register your nodes by using the SUSE CaaSP Registration Code or by registering nodes against local SUSE Repository Mirroring Server in `~/caasp/deployment/openstack/registration.auto.tfvars`:
+
...
If you need to find an IP addresses later on you can run `terraform output` within the directory you performed the deployment from `~/caasp/deployment/openstack` directory or perform the following steps:
