...
. Use command `kubectl --namespace=kube-system edit configmap oidc-dex-config` in Terminal to access Dex ConfigMap.
...
+
. Save and exit Dex ConfigMap with command ":wq" in the Terminal.
+
. Restart Dex and Gangway:
+
----
