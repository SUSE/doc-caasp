...
To ensure transparent transition and seamless compability with workloads running
on the previous CaaS Platforms, the default configuration created is using CRI-O
Container Engine in conjunction with Docker Linux capabilities. This means CaaSP v4.0
containers will run on top of CRI-O with the additional Linux capabilities: `audit_write`, `setfcap` and `mknod`.

In case you value a vanilla CRI-O experience more than out-of-the-box Docker compatibility,
this can be achieved by using the `--strict-cap-defaults` option. In that case,
the configuration created by `skuba cluster init --strict-cap-defaults`
might not be compatible with your previously running workloads,
unless you explicitly define any additional Linux capabilities they require
on top of CRI-O defaults.

[IMPORTANT]
====
After the bootstrap of the kubernetes cluster there will be no easy
