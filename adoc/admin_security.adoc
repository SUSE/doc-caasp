...
For details, refer to https://v1-11.docs.kubernetes.io/docs/reference/access-authn-authz/admission-controllers/. 


For details about access control in {kube}
, refer to https://v1-11.docs.kubernetes.io/docs/reference/access-authn-authz/controlling-access/. 
...
objectClass: top
uid:`USERID`userPassword:`PASSWORD_HASH` <2>givenname:`FIRST_NAME` <3>sn:`SURNAME` <4>cn:`FULL_NAME` <5>mail:`E-MAIL_ADDRESS` <6>
----
<1>
...
cn:`group name`uniqueMember: uid=`member1`, <11>ou=People,dc=infra,dc=caasp,dc=local
...
, use [command]``kubectl apply -f [replaceable]``YAML_FILE````.
...
Then apply the binding with [command]``kubectl apply -f [replaceable]``YAML_FILE````.
...
{productname}
enforces TLS encryption of all communications.
Administrators have the option to use the default self-signed CA (Certificate Authority) certificate or supply their own trust chain - CA certificate(s), client SSL certificate(s) and matching private key(s) - to secure all external communication with the cluster. 

[[_sec.admin.security.certs.suse_certs]]
=== SUSE-Provided Certificates


By default, {productname}
generates a self-signed CA certificate during installation which is used to authenticate and verify all communications.
...
[[_sec.admin.security.certs.usercerts]]
=== User-Provided Certificates

{productname}
administrators can opt to supply their own trust chain which includes CA certificate(s), SSL certificate(s) and matching private key(s). This is meant to secure all external administrative communication via {dashboard}
, the Kubernetes API, and Dex (authentication service). 

[[_sec.admin.security.certs.usercerts.ca]]
==== Upload CA Certificate(s)


CA certificate(s) must first be supplied in {dashboard}
at menu:Settings → External Certificates[]
; this can be a root CA from a public Certificate Authority and any necessary Intermediate Certificates, or a organization's self-signed CA certificate and any necessary Intermediate Certificates.
After saving the CA certificate, the user must apply this update by clicking the "Apply changes" button at top-right. 

==== Upload SSL Certificate(s) and Private Key(s)


SSL Certificates and Private Keys can be uploaded in {dashboard}
at menu:Settings → External Certificates[]
.
Certificate/key pairs can be uploaded for each of the 3 administrative external endpoints: {dashboard}
, Kubernetes API, and Dex.
Thus there are 3 pairs of "browse" fields for each of the 3 services.
Certificates and keys must be uploaded in matching pairs (i.e. {dashboard}
 certificate and key must be uploaded simultaneously). 

The certificate format must be X509, PEM-encoded, and ASCII armored (contains `-----BEGIN XXXXX-----` header and `-----END XXXXX-----` footer). 

During upload, the SSL certificate is validated, matched with the supplied key, and verified against the trust chain (supplied in <<_sec.admin.security.certs.usercerts.ca>>). 


image::external_cert_velum.png[scaledwidth=100%]
...
Then copy them into a YAML file and apply the definition by executing [command]``kubectl apply -f [replaceable]``YAML_FILE````. 
...
kind: ClusterRole
metadata:
  name: suse:caasp:psp:unprivilegedrules:
...
kind: ClusterRole
metadata:
  name: suse:caasp:psp:privilegedrules:
  - apiGroups: ['extensions']
    resources: ['podsecuritypolicies']
    verbs: ['use']
