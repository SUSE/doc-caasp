...
. Copy the file to a suitable location to modify it. Name the file `autoyast-worker.xml`.
...
Refer to the official {ay} documentation for more information: link:https://www.suse.com/documentation/sles-15/singlehtml/book_autoyast/book_autoyast.html[{ay} Guide].
+
. Host the {ay} files on a webserver reachable inside the network your are installing the cluster in.

=== Provisioning The Cluster Nodes

Once the {ay} file is available in the network of the cluster you can start deploying machines.
...
FIXME: What are the changes needed for the {ay} file to make this work as a master?
...
Provide `autoyast=https://[webserver/path/to/autoyast-master.xml]` during the {sle} 15 SP1 installation.
...
Provide `autoyast=https://[webserver/path/to/autoyast-worker.xml]` during the {sle} 15 SP1 installation.
