...
The configuration example sets one "receiver" to get notified by email when meets one of these conditions:
...
. Configures alerting receiver in Alertmanager
+
...
+
Replace the empty set of rules `rules: {}` in the `serverFiles` section of the Prometheus configuration file `prometheus-config-values.yaml`.
+
