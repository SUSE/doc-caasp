...
. Create DNS entries
+
In this example, we will use a worker node with public IP `10.86.4.158` (private IP `192.168.0.5`).
+
...
# These services are exposed on the node with private IP 192.168.0.5
controller:
  service:
    enableHttp: false
    externalIPs:
