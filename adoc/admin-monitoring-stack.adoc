...
+
Please refer to xref:nginx-ingress[] on how to congifure ingress in your cluster. Deploying NGINX Ingress Controller also allows us to provide TLS termination to our services and to provide basic authentication to the Prometheus Expression browser/API.
...
+
Please refer to xref:trusted_server_certificate[] on how to sign the trusted certificate.
...
+
Please refer to xref:self_signed_server_certificate[] on how to signed the self-sigend certificate.
The `server.conf` for DNS.1 is `+prometheus.example.com+`, DNS.2 is `+prometheus-alertmanager.example.com+` and DNS.3 `+grafana.example.com+`.
+
...
[IMPORTANT]
====
In order to provide additional security by using TLS certificates, please make sure you have the xref:nginx-ingress[] installed and configured.
...
==== Alertmanager Configuration Example
Refer to xref:alertmanager_configuration_example[]
...
==== Adding Grafana Dashboards
Refer to xref:adding_grafana_dashboards[]
