...
Please refererence to <<trusted_server_certificate>> on how to signed the trusted certificate.
...
Then, import trusted certificate to {kube} cluster. In this example, trusted certificate are `monitoring.key` and `monitoring.crt`.
...
Please refererence to <<self_signed_server_certificate>> on how to signed the self-sigend certificate.
The server.conf for DNS.1 is `prometheus.example.com`, DNS.2 is `prometheus-alertmanager.example.com` and DNS.3 `grafana.example.com`.

Then, import self-signed certificate to {kube} cluster. In this example, self-signed certificate are `monitoring.key` and `monitoring.crt`.
