...
The storage must be formatted to a filesystem type supporting files larger than ``4 GB``. 
...
{prompt.root}``systemctl enable --now docker.service`` 
----
. Connect the trusted storage to the internal mirror and load the registry container image to the local filesystem. 
+
...
Add the webserver as a repo to [command]``helm``. 
...
. Download all charts from the repository to the filesystem (e.g. [path]``/tmp/charts`` ). 
+ 
...
NOTE: The [command]``helm-mirror`` tool will attempt to render and inspect all downloaded charts.
...
. Download all the referenced images using [command]``skopeo``. 
+
...
----
+ 
[command]``skopeo`` will automatically create a directory named after the hostname of the registry from which you are downloading the images.
...
.
Moreover, all the available updates are going to be installed and there is no need to install updates using [command]``transactional-update`` right after the installation. 
