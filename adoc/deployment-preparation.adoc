...
master nodes of the cluster. There are many ways to configure a load balancer.
...
The load balancer should be configure before the actual deployment. It is needed
...
The load balancer needs access to port `6443` on all master nodes in the cluster.
