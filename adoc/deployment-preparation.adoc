...
This means skuba has to be installed on this host.
...
Another possibility is to connect to the nodes through a bastion using skuba
directly. This means skuba does not have to be installed on the bastion which
will just be used to forward the SSH connection. Like with the cluster nodes, the
authentication is done using *SSH keys loaded into an SSH agent* (see above).

[NOTE]
====
Make sure to have the parameter `AllowTcpForwarding` set to `yes` in `/etc/ssh/sshd_config`.
This is enabled by default so this is enabled when the option it commented.
====
