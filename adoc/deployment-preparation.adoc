...
This key pair needs to be trusted by the user you will log in as upon each cluster node; this user defaults to
"sles".  In order to use the installation tools `terraform` and `skuba`, this trusted keypair also must be
loaded into the SSH agent.
