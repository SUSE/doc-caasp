...
To add an SSH keys manually, use the `ssh-add` command:
+
----
ssh-add <path_to_key>
----
...
[TIP]
====
If you are adding the SHH key manually, specify the full path.
For example: `/home/sles/.ssh/id_rsa`
====

You can load multiple keys into your agent using the `ssh-add <path_to_key>` command.
Keys should be password protected for security measures. The
