...
{productname} requires a load balancer to distribute work load between the deployed
...
needed for the cluster nodes and preconfigure these in the load balancer.
...
We recommend performing regular HTTPS health checks for each master node `/healthz`
