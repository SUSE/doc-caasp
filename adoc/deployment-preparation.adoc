...
Expose environment variable http_proxy
----
export http_proxy=http://PROXY_IP_FQDN:PROXY_PORT
----
Replace PROXY_IP_FQDN with the ip address or fully qualified domain name of the proxy server and PROXY_PORT with its port.

If proxy server with basic authentication, create the file `/root/.curlrc` with the content:
----
