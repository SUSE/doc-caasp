...
*In order to successfully deploy {productname}, you need to have an SSH agent.* This is important, because it is required in order to use the installation tools `skuba` and `terraform`.
...
This key pair needs to be trusted by the user you will log in as into each cluster node; this user defaults to "sles".
