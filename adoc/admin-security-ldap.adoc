...
The 389 Directory Server image `registry.suse.com/caasp/v4/389-ds:beta` will automatically generate self-signed certificate and key. 389 Directory Server can be deployed with customized configuration and container command:
...
. Execute `docker` command to deploy 389 Directory Server. This will start non-TLS port (389) and TLS port (636) protocols together with automatically self-signed certificate and key.
+
----
docker run -d \
	-p 389:389 \
