...
. Execute `docker` command to deploy 389 Directory Server. This will start a non-TLS port (389) and a TLS port (636) together with an automatically self-signed certificate and key.
+
----
docker run -d \
	-p 389:389 \
