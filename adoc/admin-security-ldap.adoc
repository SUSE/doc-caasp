...
. Execute `docker` command to deploy 389 Directory Server in same Termimal. This will start a non-TLS port (389) and a TLS port (636) together with an automatically self-signed certificate and key.
+
----
docker run -d \
	-p 389:389 \
...
# CERT.PK12     Output certifcate in pkcs12
