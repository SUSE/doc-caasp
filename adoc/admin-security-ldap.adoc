...
. List the certificates in `/etc/dirsrv/slapd-localhost`:
+
----
certutil -L -d /etc/dirsrv/slapd-localhost -f /etc/dirsrv/slapd-localhost/pwdfile.txt
----
+
...
. List the key in `/etc/dirsrv/slapd-localhost`:
+
----
...
. Delete the existing CA and certificate in `/etc/dirsrv/slapd-localhost`:
+
----
certutil -D -d /etc/dirsrv/slapd-localhost -n Self-Signed-CA
certutil -F -k 6d4f98efe45328ff852b7e2ca5a24ad46844163a -d /etc/dirsrv/slapd-localhost -f /etc/dirsrv/slapd-localhost/pwdfile.txt
----

. Import the certificate into `/etc/dirsrv/slapd-localhost`:
+
Replace <KEY.PEM> with the input key.
+
Replace <CERT.PEM> with the input certificate.
+
Replace <CERT.PK12> with the output certificate in pkcs12.
+
----
openssl pkcs12 -export -inkey <KEY.PEM> -in <CERT.PEM> -out <CERT.PK12> -nodes -name "Server-Cert"
pk12util -i <CERT.PK12> -d /etc/dirsrv/slapd-localhost
----

. Import the rootCA into `/etc/dirsrv/slapd-localhost` .
+
----
certutil -A -d /etc/dirsrv/slapd-localhost -n "CA certificate" -t "CT,," -i <rootCA.pem>
----
