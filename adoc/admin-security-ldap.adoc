...
- The error message is actually a warning for 1.4.0 389ds when replace external certificates. 
+
----
...
----
+
It is due to encrypted the key is stored in the `dse.ldif`. When replacing the key and certificate in `/data/config`, 389ds will search in `dse.ldif` for symmetric key and create one if not exist. Per 389ds plan, there is a fix planned but it's to to rely on the `nssdb` material solely.
