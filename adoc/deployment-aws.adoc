...
. Find the {tf} template files for AWS in `/usr/share/caasp/terraform/aws`.
...
the directory you performed the deployment from `~/my-deployment` directory or
perform the following steps:

. Log in to the AWS Console and click on the Load Balancers. Find the one with the
string you entered in the terraform configuration above e.g. "testing-lb".
. Note down the "DNS name".
+
. Now click on Instances.
. Using the `Filter` textbox, enter the string you specified for `stack_name`
