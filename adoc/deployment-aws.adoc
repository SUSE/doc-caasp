...
NOTE: Known issue: the current terrarform script does not properly create IAM resources, instead please follow the instruction in <<aws-iam-profile-cli>> to create IAM role, role policy, and instance profile. This limitation will be addressed in the following release. 
