...
For the case that the {kube} cluster use external storage _and_ the external stroage would not handle volume snapshot by itself (either external storage does not support volume snapshot _or_ adminstrator want use velero to take volume snapshot when velero do cluster backup).
