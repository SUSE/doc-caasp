...
TIP: For complete instructions on how to install Helm and Tiller refer to xref:helm_tiller_install[].
...
+
Please refer to xref:trusted_server_certificate[] on how to sign the trusted certificate. In this example, crt and key are generated by a self-signed certificate.
+
[source,bash]
----
