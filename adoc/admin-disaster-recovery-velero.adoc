...
[IMPORTANT]
Please be well-known what persistent volumes are supported for snapshot by storage providers. Then administrators have to determine whether to deploy the _restic daemonset_ or not.
...
.. S3-compatible Storage Providers, like Minio
+
Create the credential file `credentials-velero` in the local machine.
+
```
