////
Linux
////

:grub: GRUB 2
:grub-efi: GRUB 2 for EFI
:gnome: GNOME

:xgeneric: X Window System
:xvendor: X.Org
:gnomecc: {gnome} control center
:nm: NetworkManager
:fspot: Shotwell
:yelp: Help

////
Applications
////

:kiwi: KIWI
:nautilus: {gnome} Files
:musicplayer: Banshee
:musicplayerreg: Banshee(TM)
:aa: <phrase xmlns='http://docbook.org/ns/docbook'>AppArmor</phrase>
:aareg: <phrase xmlns='http://docbook.org/ns/docbook'>AppArmor(R)</phrase>
:sudo: <command xmlns='http://docbook.org/ns/docbook'>sudo</command>
:pk: PolKit
:mariadb: Maria DB
:selnx: SELinux
:postgresql: PostgreSQL
:mysql: MariaDB
:jeos: JeOSJust enough OS
:stap: SystemTap
:oprof: OProfile
:cpufreq: CPUfreq
:powertop: powerTOP
:gpg: GPG or GnuGP
:kexec: Kexec
:kdump: Kdump
:kprobes: Kprobes
:salt: Salt
:kube: Kubernetes
:helm: Helm
:tiller: Tiller
:ostack: OpenStack



////
Company and Products
////

:suse: SUSE
:novell: Novell
:suselinux: {suse} Linux

:cap: Cloud Application Platform
:scf: {suse} Cloud Foundry

:obs: Open Build Service
:obsa: OBS

:scc: {suse} Customer Center
:sccreg: {suse}(R) Customer Center
:sccurl: https://scc.suse.com/

:yast: YaST
:yastcc: <guimenu xmlns='http://docbook.org/ns/docbook'>{yast} Control Center</guimenu>
:ycc_runlevel: Services Manager
:ycc_services_manager: Services Manager
:ay: AutoYaST

:susefirewall: SuSEFirewall2
:susefirewallfiles: SuSEfirewall2
:suseconnect: SUSEConnect

////
Products
////

:opensuse: openSUSE
:opensusereg: openSUSE(R)
:sle: SUSE Linux Enterprise
:slea: SLE
:slereg: SUSE(R) Linux Enterprise
:slert: {sle} Real Time
:slerta: {sle} RT
:slerte: {slert} Extension
:slertreg: {slereg} Real Time
:slertereg: {slereg} Real Time Extension
:sls: SUSE Linux Enterprise Server
:slsa: SLES
:slsreg: SUSE(R) Linux Enterprise Server
:slemm: {sle} Maintenance Model
:sled: SUSE Linux Enterprise Desktop
:sleda: SLED
:sledreg: SUSE(R) Linux Enterprise Desktop
:sdk: SUSE Software Development Kit
:slreg: SUSE(R) Linux
:slepos: SUSE Linux Enterprise Point of Service
:sleposreg: SUSE(R) Linux Enterprise Point of Service
:hasi: High Availability Extension
:hageo: GEO Clustering for {sle} {hasi}
:susemgr: {suse} Manager
:smtool: Subscription Management Tool
:smt: SMT
:rmtool: Repository Mirroring Tool
:rmt: RMT
:mos: MicroOS
:soc: {suse} {ostack} Cloud
:ses: {suse} Enterprise Storage

////
Product-specific
////

:productname: {suse} CaaS Platform
:productnamereg: {suse}(R) CaaS Platform
:productnumber: 3

:github_url: https://github.com/SUSE/doc-caasp


////
User/Group Names
////


////
Use exampleuser in text
Use exampleuser_plain in screen, command, option and prompt
////

:exampleuserfull: Tux Linux
:exampleuserid: tux
:exampleuser: <systemitem xmlns='http://docbook.org/ns/docbook' class='username'>{exampleuserid}</systemitem>

:exampleuser2full: Wilber Fox
:exampleuser2id: wilber
:exampleuser2: <systemitem xmlns='http://docbook.org/ns/docbook' class='username'>{exampleuser2id}</systemitem>

:exampleuser3full: Suzanne Geeko
:exampleuser3id: geeko
:exampleuser3: <systemitem xmlns='http://docbook.org/ns/docbook' class='username'>exampleuser3id</systemitem>

:examplegroup_plain: users
:examplegroup: <systemitem xmlns='http://docbook.org/ns/docbook' class='groupname'>users</systemitem>


:rootuser: <systemitem xmlns='http://docbook.org/ns/docbook' class='username'>root</systemitem>




:ovs: <phrase role='productname' xmlns='http://docbook.org/ns/docbook'>Open vSwitch</phrase>
:netteam: Network Teaming

// daemons
:systemd: <systemitem xmlns='http://docbook.org/ns/docbook' class='daemon'>systemd</systemitem>
:crond: <systemitem xmlns='http://docbook.org/ns/docbook' class='daemon'>cron</systemitem>
:oprofd: <systemitem xmlns='http://docbook.org/ns/docbook' class='daemon'>oprofile</systemitem>

////
Virtualization
////

:xenstore: XenStore
:lxc: LXC
:xen: Xen
:xenreg: Xen(R)
:kvm: KVM
:vmware: VMware
:vmhost: VM Host Server
:vmguest: VM Guest
:dom0: Dom0
:lvmcache: <systemitem xmlns='http://docbook.org/ns/docbook' class='resource'>lvmcache</systemitem>
:bcache: <systemitem xmlns='http://docbook.org/ns/docbook' class='resource'>bcache</systemitem>
:libvirt: <systemitem xmlns='http://docbook.org/ns/docbook' class='library'>libvirt</systemitem>
:libvirtd: <systemitem xmlns='http://docbook.org/ns/docbook' class='daemon'>libvirtd</systemitem>
:vmm: Virtual Machine Manager
:qemu: QEMU
:qemusystemarch: <command xmlns='http://docbook.org/ns/docbook'>qemu-system-<replaceable>ARCH</replaceable></command>
:pciback: PCI Pass-Through
:usbback: USB Pass-Through
:vgaback: VGA Pass-Through
:lvs: Linux Virtual Server

////
HA
////

:ha: High Availability
:ais: OpenAIS
:stonith: STONITH

////
Books
////

:deploy: <citetitle xmlns='http://docbook.org/ns/docbook'>Deployment Guide</citetitle>
:instquick: <citetitle xmlns='http://docbook.org/ns/docbook'>Installation Quick Start</citetitle>
:admin: <citetitle xmlns='http://docbook.org/ns/docbook'>Administration Guide</citetitle>
:user: <citetitle xmlns='http://docbook.org/ns/docbook'>User Guide</citetitle>
:reference: <citetitle xmlns='http://docbook.org/ns/docbook'>Reference</citetitle>
:startup: <citetitle xmlns='http://docbook.org/ns/docbook'>Start-Up</citetitle>

////
Book Abstracts
////

:abstract_installquick: Lists the system requirements and guides you step-by-step through the installation of {productname} from DVD, or from an ISO image.

////
Platforms
////

:x86: x86
:amd64: AMD64
:zseries: z{nbsp}Systems
:ipseries: POWER
:ppc: POWER
:power: POWER
:intel64: Intel{nbsp}64
:x86-64: {amd64}/{intel64}
:arm: ARM
:aarch64: AArch64
:arm64: {arm}{nbsp}{aarch64}

:powerlinux: PowerLinux
:powerkvm: PowerKVM

:mac: <replaceable xmlns='http://docbook.org/ns/docbook'>MAC</replaceable>
:hwmac: hwtype.<replaceable xmlns='http://docbook.org/ns/docbook'>MAC</replaceable>
:confmac: config.<replaceable xmlns='http://docbook.org/ns/docbook'>MAC</replaceable>
:btrfs: Btrfs


////
Miscellaneous
////

:ptp: Precision Time Protocol
:ntp: Network Time Protocol
:altf2: kbd:[Alt+F2]
:admin_node: administration node
:Admin_node: Administration node
:Admin_Node: Administration Node
:master_node: master node
:Master_node: Master node
:Master_Node: Master Node
:worker_node: worker node
:Worker_node: Worker node
:Worker_Node: Worker Node
:cluster_node: cluster node
:Cluster_node: Cluster node
:Cluster_Node: Cluster Node
:smaster: {salt} master
:sminion: {salt} minion
:dashboard: Velum
:kmaster: {kube} master
:kworker: {kube} worker
:kubeconfig: <filename xmlns='http://docbook.org/ns/docbook'>kubeconfig</filename>
:kubectl: <command xmlns='http://docbook.org/ns/docbook'>kubectl</command>
:tupdate: <command xmlns='http://docbook.org/ns/docbook'>transactional-update</command>
:caasp-cli: <command xmlns='http://docbook.org/ns/docbook'>caasp-cli</command>
:kubedoc: https://v1-11.docs.kubernetes.io/docs/

////
Prompts
////

:prompt_root: <prompt role='root' xmlns='http://docbook.org/ns/docbook'>root # </prompt>
:prompt_root_admin: <prompt role='root' xmlns='http://docbook.org/ns/docbook'>root@admin # </prompt>
:prompt_root_master: <prompt role='root' xmlns='http://docbook.org/ns/docbook'>root@master # </prompt>
:prompt_user: <prompt xmlns='http://docbook.org/ns/docbook'>{exampleuserid} {gt} </prompt>
:prompt_user2: <prompt xmlns='http://docbook.org/ns/docbook'>{exampleuser2id} {gt} </prompt>
:prompt_sudo: <prompt xmlns='http://docbook.org/ns/docbook'>{exampleuserid} {gt} sudo </prompt>
:prompt_bash: <prompt xmlns='http://docbook.org/ns/docbook'>bash-4.3 # </prompt>
