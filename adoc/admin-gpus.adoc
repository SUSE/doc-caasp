...
In the Grafana web interface, navigate to Manage Dashboards and click `Import`. In the field "Import via grafana.com", enter the dashboard ID `12219`, and click `Load`.

Alternatively, download the link:https://raw.githubusercontent.com/NVIDIA/gpu-monitoring-tools/2.0.0-rc.11/grafana/dcgm-exporter-dashboard.json[dashboard JSON definition], and upload it with the "Upload .json file" button.

On the next page, in the dropdown menu labeled "Prometheus", select "Prometheus" as the data source. Customize the dashboard name and UID to your preference. Then click "Import".
