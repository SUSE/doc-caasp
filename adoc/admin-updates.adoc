...
These procedures differ for *patch updates* (for example 1.16.1 to 1.16.2), which are handled by `skuba-update` as described in xref:_base_os_updates[].
...
After that, rebooting the node will depend on whether you have also disabled reboots or not. If you have disabled reboots for this node, then you will have to follow the instructions as given in xref:Completely Disabling Reboots[]. Otherwise, you will have to wait until `kured` performs the reboot of the node
