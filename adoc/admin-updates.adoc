...
After you have done this, you will have to manage reboots yourself. In order to do this, you will have to follow some steps whenever you want to issue a reboot for a node. First of all, you will have to `cordon` and `drain` the node:

----
kubectl cordon NODE
kubectl drain --force=true --delete-local-data=false --ignore-daemonsets=false NODE
----

Only then you will be able to manually `reboot` the node safely. Whenever the node is back, remember to `uncordon` it so it is scheduleable again:
...
Bear in mind that you do not have to do all this if the node to be rebooted does not contain any workload whatsoever.
