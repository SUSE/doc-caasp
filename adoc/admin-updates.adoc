...
kubectl drain --force=true \
  ...
  --delete-local-data=false \ // <1>
  --grace-period 600 \ // <2>
  --timeout=900s \ // <3>
  <NODE_ID>
----
<1> Continues even if there are pods using `emptyDir` (local data that will be deleted when the node is drained).
<2> Running applications will be notified of termination and given 10 minutes (`600` seconds) to safely store data.
<3> Draining of the node will fail after 15 minutes (`900` seconds) have elapsed without success.
