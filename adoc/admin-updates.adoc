...
[NOTE]
===
`skuba-update` uses the flags `--non-interactive --non-interactive-include-reboot-patches` when running. `--non-interactive-include-reboot-patches` when used in conjunction with `--non-interactive` this will treat patches with the `rebootSuggested-flag` as non-interactive as the reboot will be managed by `kured`.
===
