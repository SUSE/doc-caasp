...
Before upgrading a {productname} cluster, it's required to upgrade packages installed by the `SUSE-CaaSP-Management` pattern on the management workstation.

The cluster upgrade depends on updated skuba, but might also require new helm / {tf} or other dependencies which will be updated with the refreshed pattern.
