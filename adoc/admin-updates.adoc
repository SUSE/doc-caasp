...
[NOTE]
===
`skuba-update` uses the flags `--non-interactive --non-interactive-include-reboot-patches`. `--non-interactive-include-reboot-patches`, when used in conjunction with `--non-interactive`, will cause patches with the `rebootSuggested-flag` not to be skipped. The reboot will be managed by `kured`.
===
